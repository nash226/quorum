...
test("evaluates a shipped HR equipment return fixture across separation claims", async () => {
  const scorecard = await evaluateFixtureFile({
    fixturePath: resolve("examples/evaluations/hr/equipment-return-policy.json"),
    generatedAt: "2026-07-29T12:00:00.000Z",
  });

  assert.equal(scorecard.fixtureName, "HR equipment return policy example");
  assert.equal(scorecard.domain, "hr");
  assert.deepEqual(scorecard.actualSummary, {
    verified: 1,
    contradicted: 1,
  ...
  assert.equal(scorecard.report.sources[0]?.id, "people-ops/hr-equipment-return@2026-07-29");
  assert.equal(scorecard.summaryMatches, true);
  assert.equal(scorecard.score, 1);
});
...
test("resolves fixture paths from nested directories in stable order", async () => {
  ...
  assert.deepEqual(fixturePaths, [
    ...
    resolve("examples/evaluations/hr/commuter-benefit-policy.json"),
      ...
      resolve("examples/evaluations/hr/equipment-return-policy.json"),
...
test("evaluates fixture files from explicit paths and fixture directories", async () => {
  ...
  assert.equal(scorecards.length, 94);
  assert.deepEqual(
    ...
    [
      ...
      "HR equipment return policy example",
...
test("filters evaluation fixture files by domain", async () => {
  ...
  assert.equal(scorecards.length, 34);
  assert.deepEqual(
    scorecards.map((scorecard) => scorecard.fixtureName),
    [
      "HR policy example",
