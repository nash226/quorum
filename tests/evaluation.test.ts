...
test("evaluates the shipped HR recognition fixture across award claims", async () => {
  ...
  assert.equal(scorecard.fixtureName, "HR recognition policy example");
  assert.equal(scorecard.domain, "hr");
  assert.equal(scorecard.answerLabel, "HR recognition reviewer packet");
  ...
  assert.equal(scorecard.report.sources[0]?.id, "people-ops/hr-recognition@2026-08-03");
...
test("evaluates the shipped support knowledge-base access fixture across access claims", async () => {
  const scorecard = await evaluateFixtureFile({
    fixturePath: resolve("examples/evaluations/support/knowledge-base-access-policy.json"),
    generatedAt: "2026-08-08T12:00:00.000Z",
  });

  assert.equal(scorecard.fixtureName, "Support knowledge-base access policy example");
  assert.equal(scorecard.domain, "support");
  assert.equal(scorecard.answerLabel, "Support knowledge-base access reviewer packet");
  assert.deepEqual(scorecard.actualSummary, {
    verified: 1,
    contradicted: 1,
    unsupported: 1,
  ...
  assert.equal(scorecard.report.sources[0]?.id, "support/knowledge-base-access@2026-08-08");
  assert.equal(scorecard.summaryMatches, true);
  assert.equal(scorecard.score, 1);
});
...
test("evaluates the shipped support plan-upgrade fixture across billing claims", async () => {
  ...
  assert.equal(scorecard.fixtureName, "Support plan upgrade policy example");
  assert.equal(scorecard.domain, "support");
  assert.deepEqual(scorecard.actualSummary, {
    verified: 1,
    contradicted: 0,
...
test("evaluates a shipped HR employee assistance fixture across benefit claims", async () => {
  const scorecard = await evaluateFixture({
    ...
    answer: "Employees receive 8 confidential counseling sessions per year.\nEmployees receive 80 confidential counseling sessions per year.\nThe company pays for private therapy indefinitely.\n",
...
test("evaluates a shipped HR workplace safety fixture across compliance claims", async () => {
  ...
  assert.equal(scorecard.fixtureName, "HR workplace safety policy example");
  assert.equal(scorecard.answerLabel, "HR workplace safety reviewer packet");
  assert.deepEqual(scorecard.actualSummary, { verified: 1, contradicted: 0, unsupported: 1, needs_review: 1 });
  assert.deepEqual(scorecard.claims.map((claim) => claim.actualVerdict), ["verified", "needs_review", "unsupported"]);
  assert.equal(scorecard.report.sources[0]?.id, "people-ops/hr-workplace-safety@2026-07-29");
  assert.equal(scorecard.summaryMatches, true);
  assert.equal(scorecard.score, 1);
});
...
test("evaluates a shipped support invoice correction fixture across policy claims", async () => {
  const scorecard = await evaluateFixtureFile({
...
test("evaluates a shipped support price adjustment fixture across policy claims", async () => {
  const scorecard = await evaluateFixtureFile({
    fixturePath: resolve("examples/evaluations/support/price-adjustment-policy.json"),
  ...
  assert.equal(scorecard.fixtureName, "Support price adjustment policy example");
  assert.equal(scorecard.domain, "support");
  assert.equal(scorecard.answerLabel, "Support price adjustment reviewer packet");
  assert.deepEqual(scorecard.actualSummary, { verified: 2, contradicted: 0, unsupported: 0, needs_review: 1 });
  assert.deepEqual(scorecard.claims.map((claim) => claim.actualVerdict), ["verified", "needs_review", "verified"]);
  assert.equal(scorecard.report.sources[0]?.id, "support/price-adjustment@2026-07-29");
  assert.equal(scorecard.summaryMatches, true);
  assert.equal(scorecard.score, 1);
});
...
test("evaluates a shipped HTML billing answer against an HTML policy", async () => {
  const scorecard = await evaluateFixtureFile({
    fixturePath: resolve("examples/evaluations/support/html-billing-answer-policy.json"),
    generatedAt: "2026-08-02T20:00:00.000Z",
  });

  assert.equal(scorecard.fixtureName, "Support HTML billing answer policy example");
  assert.equal(scorecard.domain, "support");
  assert.equal(scorecard.answerLabel, "Support HTML billing reviewer packet");
  assert.deepEqual(scorecard.actualSummary, {
    verified: 1,
    contradicted: 1,
    unsupported: 1,
  ...
  assert.equal(scorecard.report.sources[0]?.title, "Billing and Support Policy");
  assert.equal(scorecard.summaryMatches, true);
  assert.equal(scorecard.score, 1);
});
...
test("evaluates a shipped support renewal notice fixture across policy claims", async () => {
  const scorecard = await evaluateFixtureFile({
  ...
  assert.equal(scorecard.domain, "support");
  assert.equal(scorecard.answerLabel, "Support renewal notice reviewer packet");
  assert.deepEqual(scorecard.actualSummary, {
    verified: 1,
    contradicted: 1,
    unsupported: 1,
...
test("evaluates a shipped support replacement fixture across eligibility claims", async () => {
  const scorecard = await evaluateFixtureFile({
...
test("evaluates a shipped fixture that discovers approved sources from a directory", async () => {
  ...
  assert.deepEqual(scorecard.sourcePaths, [
    ...
    resolve("examples/sources/hr-recognition-policy.md"),
    ...
    resolve("examples/sources/support-replacement-policy.md"),
...
test("resolves fixture paths from nested directories in stable order", async () => {
  ...
  assert.deepEqual(fixturePaths, [
    ...
    resolve("examples/evaluations/hr/overtime-policy.json"),
    ...
    resolve("examples/evaluations/hr/recognition-policy.json"),
    ...
    resolve("examples/evaluations/support/html-billing-answer-policy.json"),
    ...
    resolve("examples/evaluations/support/knowledge-base-access-policy.json"),
    ...
    resolve("examples/evaluations/support/plan-upgrade-policy.json"),
    ...
    resolve("examples/evaluations/support/security-controls-policy.json"),
...
test("evaluates fixture files from explicit paths and fixture directories", async () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      "HR travel expense policy example",
      ...
      "Support HTML billing answer policy example",
      ...
      "Support knowledge-base access policy example",
      ...
      "Support plan upgrade policy example",
      ...
      "Support security controls policy example",
...
test("matches evaluation domains case-insensitively", async () => {
  const scorecards = await evaluateFixtureFiles({
    fixtureDirPaths: [resolve("examples/evaluations")],
    fixturePaths: [],
    domains: [" HR "],
