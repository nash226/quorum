...
test("evaluates authorized-contact controls in the support benchmark", async () => {
  const scorecard = await evaluateFixtureFile({
  ...
  assert.equal(scorecard.summaryMatches, true);
  assert.equal(scorecard.report.sources[0]?.id, "support/authorized-contact@2026-07-15");
  assert.equal(scorecard.score, 1);
});
