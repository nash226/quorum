...
test("evaluates a shipped HR employee recognition fixture across award claims", async () => {
  const scorecard = await evaluateFixtureFile({
    fixturePath: resolve("examples/evaluations/hr/recognition-policy.json"),
    generatedAt: "2026-07-29T07:00:00.000Z",
  });

  assert.equal(scorecard.fixtureName, "HR employee recognition policy example");
  assert.equal(scorecard.domain, "hr");
  assert.equal(scorecard.answerLabel, "HR employee recognition reviewer packet");
  assert.deepEqual(scorecard.actualSummary, {
    verified: 2,
    contradicted: 0,
    unsupported: 0,
  ...
  assert.equal(scorecard.report.sources[0]?.id, "people-ops/hr-recognition@2026-07-29");
  assert.equal(scorecard.summaryMatches, true);
  assert.equal(scorecard.score, 1);
});
...
test("evaluates fixture files from explicit paths and fixture directories", async () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      "HR employee recognition policy example",
