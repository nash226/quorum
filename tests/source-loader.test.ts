...
test("strips supported text extensions from fallback source titles", async () => {
  ...
  const yamlSource = await sourceDocumentFromFile(
    "docs/policies/benefits-policy.yaml",
    "parental_leave_weeks: 12",
    2,
  );
  const ymlSource = await sourceDocumentFromFile(
    "docs/policies/refunds-policy.yml",
  ...
  assert.equal(yamlSource.title, "benefits-policy");
  assert.equal(ymlSource.title, "refunds-policy");
});
