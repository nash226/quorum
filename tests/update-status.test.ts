...
test("loadRecentShipments reads the supplied history ref", () => {
  ...
  const latestShipmentCommit = execFileSync(
    "git",
    [
      "log",
      "--first-parent",
  ...
  assert.ok(latestShipmentCommit);

  assert.ok(shipments.length > 0);
  assert.equal(shipments[0]?.shortCommit, latestShipmentCommit.slice(0, 7));
...
test("benchmark inventory keeps HR and support domain counts visible", () => {
  ...
  assert.equal(Object.values(counts).reduce((total, count) => total + count, 0), 88);
  assert.deepEqual({ hr: counts.hr, support: counts.support }, { hr: 30, support: 58 });
  assert.match(readme, /30 HR and 58 support workflows/);
