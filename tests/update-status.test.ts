...
test("benchmark inventory keeps HR and support domain counts visible", () => {
  ...
  assert.equal(Object.values(counts).reduce((total, count) => total + count, 0), 93);
  assert.deepEqual({ hr: counts.hr, support: counts.support }, { hr: 32, support: 61 });
  assert.match(readme, /32 HR and 61 support workflows/);
