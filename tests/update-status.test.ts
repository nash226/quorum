...
test("benchmark inventory keeps HR and support domain counts visible", () => {
  ...
  assert.equal(Object.values(counts).reduce((total, count) => total + count, 0), 87);
  assert.deepEqual({ hr: counts.hr, support: counts.support }, { hr: 30, support: 57 });
  assert.match(readme, /30 HR and 57 support workflows/);
