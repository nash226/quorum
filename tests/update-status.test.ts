...
test("status refresh reads capabilities from the CLI guide", () => {
  ...
  assert.match(status, /^- read Markdown\/MDX\/Quarto, AsciiDoc, Org-mode, MediaWiki, text, HTML\/XHTML, JSON, YAML\/YML, XML, TOML, PDF, and DOCX answers and approved sources$/m);
...
test("benchmark inventory keeps HR and support domain counts visible", () => {
  ...
  assert.equal(Object.values(counts).reduce((total, count) => total + count, 0), 79);
  assert.deepEqual({ hr: counts.hr, support: counts.support }, { hr: 27, support: 52 });
  assert.match(readme, /27 HR and 52 support workflows/);
});
