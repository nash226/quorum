...
test("benchmark inventory keeps HR and support domain counts visible", () => {
  ...
  assert.equal(Object.values(counts).reduce((total, count) => total + count, 0), 81);
  assert.deepEqual({ hr: counts.hr, support: counts.support }, { hr: 28, support: 53 });
  assert.match(readme, /28 HR and 53 support workflows/);
});
