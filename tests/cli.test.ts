...
test("verify accepts a direct HTM answer export", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-htm-answer-"));

  try {
    const answerPath = join(tempDir, "answer.htm");
    const sourcePath = join(tempDir, "policy.md");
    await Promise.all([
      writeFile(answerPath, "<html><body><p>Employees receive 12 weeks of paid parental leave.</p></body></html>\n", "utf8"),
      writeFile(sourcePath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
