...
test("review-queue combines reviewer workload and benchmark drift", async () => {
  ...
  try {
    ...
    assert.match(text, /Final verdicts: 26 verified, 16 contradicted, 25 unsupported, 25 needs review/);
...
test("review-queue scopes workload to a queue status", async () => {
  ...
  try {
    ...
    assert.deepEqual(pendingOverview.review, {
      ...
      pendingClaims: 92,
      reviewedClaims: 0,
      verdicts: { verified: 26, contradicted: 16, unsupported: 25, needs_review: 25 },
    });
