...
test("verify-batch discovers common Markdown extension answers", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-markdown-extensions-"));
  ...
  try {
    ...
    await Promise.all([
      ...
      mkdir(sourceDir, { recursive: true }),
      writeFile(join(answerDir, "answer.mdown"), "Employees receive 12 weeks of leave.\n", "utf8"),
      writeFile(join(answerDir, "nested.mkdn"), "Refunds are available within 30 days.\n", "utf8"),
    ...
    assert.deepEqual(report.answers.map((answer) => answer.answerPath), [
      join(answerDir, "answer.mdown"),
      join(answerDir, "nested.mkdn"),
    ]);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
