...
test("help advertises the complete discovered input contract", async () => {
  ...
  assert.match(stdout, /Answers: Markdown\/MDX\/Quarto, AsciiDoc\/Org, MediaWiki, reStructuredText, LaTeX, text, HTML\/XHTML, JSON\/JSONL\/NDJSON, YAML, XML, CSV\/TSV, PDF, DOCX, and TOML/);
  assert.match(stdout, /Sources: Markdown\/MDX\/Quarto, AsciiDoc\/Org, MediaWiki, reStructuredText, LaTeX, text, HTML\/XHTML, JSON\/JSONL\/NDJSON, YAML, XML, CSV\/TSV, PDF, DOCX, and TOML/);
});
...
test("evaluate writes a one-row-per-domain summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /^[^,\n]+,support,52,0,0\.000,51,1,152,152,1\.000,100%,57,34,44,17,57,34,44,17$/m);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes a one-row aggregate summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /,79,78,1,0,0\.000,234,234,1\.000,100%,hr .*89,53,66,26,89,53,66,26/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes the gate-aware result JSON to disk", async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.fixtureCount, 79);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("review-queue combines reviewer workload and benchmark drift", async () => {
  ...
  try {
    ...
    assert.equal(overview.evaluation.fixtureCount, 79);
    ...
    assert.match(text, /Benchmark drift: 0\/79 mismatches \(0%\)/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
