...
test("evaluate writes a one-row-per-domain summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /^[^,\n]+,support,48,0,0\.000,47,1,140,140,1\.000,100%,53,33,41,16,53,33,41,16$/m);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes a one-row aggregate summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /,72,71,1,0,0\.000,213,213,1\.000,100%,hr \| support,24 \| 48,0 \| 0,0\.000 \| 0\.000,24 \| 47,0 \| 1,1\.000 \| 1\.000,100% \| 100%,82,49,60,25,82,49,60,25/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("review-queue combines reviewer workload and benchmark drift", async () => {
  ...
  try {
    ...
    assert.match(text, /Final verdicts: 26 verified, 17 contradicted, 25 unsupported, 27 needs review/);
