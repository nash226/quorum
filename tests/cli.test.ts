...
test("evaluate filters shipped example fixtures by domain", async () => {
  ...
  assert.match(stdout, /Fixtures: 34/);
});
...
test("evaluate writes a one-row-per-domain summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /^[^,\n]+,hr,34,0,0\.000,34,0,103,103,1\.000,100%,40,22,29,12,40,22,29,12$/m);
...
test("evaluate writes a one-row aggregate summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /,94,93,1,0,0\.000,280,280,1\.000,100%,hr .*106,61,80,33,106,61,80,33/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes the gate-aware result JSON to disk", async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.fixtureCount, 94);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("review-queue combines reviewer workload and benchmark drift", async () => {
  ...
  try {
    ...
    assert.equal(overview.evaluation.fixtureCount, 94);
    ...
    assert.match(text, /Benchmark drift: 0\/94 mismatches \(0%\)/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("review-queue scopes benchmark drift to selected domains", async () => {
  ...
  try {
    ...
    assert.equal(overview.evaluation.fixtureCount, 34);
    assert.deepEqual(overview.evaluation.domains, [{ domain: "hr", fixtureCount: 34, mismatchCount: 0 }]);
    const csvOutPath = join(tempDir, "queue.csv");
