...
test("evaluate writes a one-row-per-domain summary csv", async () => {
  ...
  assert.match(summaryCsv, /^[^,\n]+,support,46,0,0\.000,45,1,135,135,1\.000,100%,51,31,39,14,51,31,39,14$/m);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes a one-row aggregate summary csv", async () => {
  ...
  assert.match(summaryCsv, /,66,65,1,0,0\.000,196,196,1\.000,100%,hr \| support,20 \| 46,0 \| 0,0\.000 \| 0\.000,20 \| 45,0 \| 1,1\.000 \| 1\.000,100% \| 100%,76,44,55,21,76,44,55,21/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("review-queue combines reviewer workload and benchmark drift", async () => {
  ...
  try {
    ...
    assert.equal(overview.review.totalAnswers, 28);
