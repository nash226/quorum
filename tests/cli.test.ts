...
test("verify accepts a direct HTML answer export", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-html-answer-"));
  ...
  try {
    ...
    await Promise.all([
      writeFile(
        answerPath,
        "<!doctype html><html><body><p>Employees receive 12 weeks of paid parental leave.</p></body></html>\n",
        "utf8",
      ),
      writeFile(sourcePath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
