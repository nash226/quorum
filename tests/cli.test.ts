...
test("verify-batch discovers LaTeX answers and sources", async () => {
  ...
  try {
    ...
    assert.equal(report.answerCount, 1);
    assert.equal(report.sourceCount, 1);
    assert.equal(report.answers[0]?.answerPath, join(answerDir, "leave.tex"));
    assert.equal(report.sources[0]?.title, "hr-policy");
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
...
test("verify-batch discovers uppercase CSV exports for answers and sources", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-uppercase-csv-discovery-"));
  ...
  try {
    ...
    await Promise.all([
      writeFile(join(answerDir, "leave.CSV"), "claim,notes\n\"Employees receive 12 weeks of paid parental leave.\",approved\n", "utf8"),
      writeFile(join(sourceDir, "hr-policy.CSV"), "claim,notes\n\"Employees receive 12 weeks of paid parental leave.\",approved\n", "utf8"),
    ]);
...
test("verify-batch discovers nested .log answers and sources", async () => {
  ...
  try {
    ...
    await mkdir(sourceDir, { recursive: true });
    await Promise.all([
      writeFile(join(answerDir, "answer.log"), "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(join(sourceDir, "policy.log"), "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
    ...
    assert.match(report.answers[0]?.answerPath ?? "", /answers\/nested\/answer\.log$/);
...
test("verify-batch discovers nested XML answers and sources", async () => {
  ...
  try {
    ...
    await Promise.all([
      writeFile(
        join(answerDir, "leave.xml"),
        '<?xml version="1.0"?><answer><claim>Employees receive 12 weeks of paid parental leave.</claim></answer>',
        "utf8",
      ),
      writeFile(
        join(sourceDir, "hr-policy.xml"),
        '<?xml version="1.0"?><policy><title>HR Benefits Policy</title><rule>Employees receive 12 weeks of paid parental leave.</rule></policy>',
        "utf8",
      ),
    ]);
    ...
    assert.equal(report.answers.length, 1);
    assert.match(report.answers[0]?.answerPath ?? "", /answers\/nested\/leave\.xml$/);
...
test("verify-batch discovers nested AsciiDoc answers and sources", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-asciidoc-discovery-"));
  ...
  try {
    ...
    await mkdir(sourceDir, { recursive: true });
    await Promise.all([
      writeFile(join(answerDir, "answer.adoc"), "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(join(sourceDir, "policy.adoc"), "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
    ...
    assert.equal(report.answers.length, 1);
    assert.match(report.answers[0]?.answerPath ?? "", /answers\/nested\/answer\.adoc$/);
    assert.deepEqual(report.sources.map((source) => source.title), ["policy"]);
    assert.equal(report.summary.verified, 1);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
...
test("verify-batch discovers nested YAML answers and sources", async () => {
  ...
  try {
    ...
    assert.equal(report.answers.length, 1);
    assert.match(report.answers[0]?.answerPath ?? "", /answers\/nested\/leave\.yml$/);
...
test("verify-batch discovers nested LaTeX answers and sources", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-latex-discovery-"));
  ...
  try {
    ...
    await Promise.all([
      writeFile(
        join(answerDir, "leave.tex"),
        "\\section{Benefits}\nEmployees receive 12 weeks of paid parental leave.\n",
        "utf8",
      ),
      writeFile(
        join(sourceDir, "hr-policy.tex"),
        "\\section{HR Benefits Policy}\nEmployees receive 12 weeks of paid parental leave.\n",
        "utf8",
      ),
    ]);
    ...
    assert.equal(report.answers.length, 1);
    assert.match(report.answers[0]?.answerPath ?? "", /answers[\\/]nested[\\/]leave\.tex$/);
    assert.match(report.sources[0]?.sourcePath ?? "", /sources[\\/]nested[\\/]hr-policy\.tex$/);
    assert.equal(report.summary.verified, 1);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
...
test("verify-batch discovers nested XHTML answers and sources", async () => {
  ...
  try {
    ...
    await mkdir(sourceDir, { recursive: true });
    await Promise.all([
      writeFile(join(answerDir, "leave.xhtml"), "<html><body><p>Employees receive 12 weeks of paid parental leave.</p></body></html>", "utf8"),
      writeFile(join(sourceDir, "hr-policy.xhtml"), "<html><body><h1>HR Benefits Policy</h1><p>Employees receive 12 weeks of paid parental leave.</p></body></html>", "utf8"),
    ]);
...
test("formats lists the extensions accepted by source and answer discovery", async () => {
  ...
  assert.match(stdout, /Source files: .*\.jsonc/);
...
test("help advertises the complete discovered input contract", async () => {
  ...
  assert.match(stdout, /Answers: Markdown\/MDX\/Quarto, AsciiDoc\/Org, MediaWiki, reStructuredText, LaTeX\/Textile, text, HTML\/XHTML, JSON\/JSONL\/NDJSON\/JSON5\/JSONC, YAML, XML, TOML, CSV\/TSV, EML\/EMLX, RTF, PDF, and DOCX/);
  assert.match(stdout, /Sources: Markdown\/MDX\/Quarto, AsciiDoc\/Org, MediaWiki, reStructuredText, LaTeX\/Textile, text, HTML\/XHTML, JSON\/JSONL\/NDJSON\/JSON5\/JSONC, YAML, XML, TOML, CSV\/TSV, EML\/EMLX, RTF, PDF, and DOCX/);
});
...
test("verify accepts paired .text answer and source exports", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-text-paired-"));

  try {
    const answerPath = join(tempDir, "answer.text");
    const sourcePath = join(tempDir, "policy.text");
    await Promise.all([
      writeFile(answerPath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(sourcePath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
...
test("verify accepts direct .org-mode answer and source exports", async () => {
  ...
  try {
    const answerPath = join(tempDir, "answer.org-mode");
    const sourcePath = join(tempDir, "policy.org-mode");
    await Promise.all([
      writeFile(answerPath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(sourcePath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
...
test("verify accepts direct .ini answer and source exports", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-ini-direct-"));

  try {
    const answerPath = join(tempDir, "answer.ini");
    const sourcePath = join(tempDir, "policy.ini");
    await Promise.all([
      writeFile(answerPath, "leave=Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(sourcePath, "leave=Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
...
test("verify accepts a direct .markdown answer and source export", async () => {
  ...
  try {
    ...
    const sourcePath = join(tempDir, "policy.markdown");
    await Promise.all([
      writeFile(answerPath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(sourcePath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
...
test("verify-batch discovers .markdown answers and sources from directories", async () => {
  ...
  try {
    ...
    await Promise.all([
      writeFile(join(answerDir, "answer.markdown"), "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(join(sourceDir, "policy.markdown"), "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
    ...
    assert.equal(report.answers.length, 1);
    assert.equal(report.answers[0]?.answerPath, join(answerDir, "answer.markdown"));
    assert.equal(report.answers[0]?.report.summary.verified, 1);
    assert.deepEqual(report.sources.map((source) => source.sourcePath), [join(sourceDir, "policy.markdown")]);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("verify-batch discovers .mdwn Markdown aliases from directories", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-mdwn-alias-"));
  const answerDir = join(tempDir, "answers");
  const sourceDir = join(tempDir, "sources");
  try {
    await mkdir(answerDir);
    await mkdir(sourceDir);
    await Promise.all([
      writeFile(join(answerDir, "leave.mdwn"), "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(join(sourceDir, "policy.mdwn"), "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
    ...
    assert.equal(report.answers[0]?.answerPath, join(answerDir, "leave.mdwn"));
    assert.equal(report.answers[0]?.report.summary.verified, 1);
    assert.equal(report.sources[0]?.sourcePath, join(sourceDir, "policy.mdwn"));
    assert.equal(report.sources[0]?.title, "policy");
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
...
test("verify accepts direct Textile answer and source exports", async () => {
  ...
  try {
    const answerPath = join(tempDir, "answer.textile");
    const sourcePath = join(tempDir, "policy.textile");
    await Promise.all([
      writeFile(answerPath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(sourcePath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
...
test("verify accepts direct Quarto answers and approved sources", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-quarto-"));

  try {
    const answerPath = join(tempDir, "answer.qmd");
    const sourcePath = join(tempDir, "policy.qmd");
    await Promise.all([
      writeFile(answerPath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(sourcePath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
...
test("verify accepts a direct CSV approved source export", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-csv-source-"));
...
test("verify accepts a direct HTML approved source export", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-html-source-"));

  try {
    const answerPath = join(tempDir, "answer.md");
    const sourcePath = join(tempDir, "policy.html");
    await Promise.all([
      writeFile(answerPath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(
        sourcePath,
        "<html><head><title>Benefits</title></head><body><h1>Leave</h1><p>Employees receive 12 weeks of paid parental leave.</p></body></html>",
        "utf8",
      ),
    ]);
...
test("verify accepts a direct Org-mode answer export", async () => {
  ...
  try {
    const answerPath = join(tempDir, "answer.org");
    const sourcePath = join(tempDir, "policy.md");
    await Promise.all([
...
test("verify-batch discovers INI answers and sources in nested directories", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-ini-batch-"));
  ...
  try {
    ...
    assert.equal(report.answerCount, 1);
    assert.equal(report.summary.verified, 1);
    assert.equal(report.answers[0]?.answerPath, join(answerDir, "answer.ini"));
    assert.equal(report.sources[0]?.sourcePath, join(sourceDir, "policy.ini"));
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("verify-batch discovers MediaWiki answers and sources", async () => {
  ...
  try {
    await Promise.all([mkdir(answerDir), mkdir(sourceDir)]);
    await Promise.all([
      writeFile(join(answerDir, "answer.mediawiki"), "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(join(sourceDir, "policy.wiki"), "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
    ...
    assert.equal(report.summary.answersWithClaims, 1);
    assert.equal(report.summary.verified, 1);
    assert.equal(report.answers[0]?.answerPath, join(answerDir, "answer.mediawiki"));
    assert.equal(report.sources[0]?.sourcePath, join(sourceDir, "policy.wiki"));
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("verify accepts direct .wiki answer and approved source exports", async () => {
  ...
  try {
    const answerPath = join(tempDir, "answer.wiki");
    const sourcePath = join(tempDir, "policy.wiki");
    await Promise.all([
      writeFile(answerPath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(sourcePath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
...
test("verify accepts the direct reStructuredText .rest answer alias", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-rest-answer-"));

  try {
    const answerPath = join(tempDir, "answer.rest");
    const sourcePath = join(tempDir, "policy.md");
    await Promise.all([
      writeFile(answerPath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(sourcePath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
...
test("verify accepts paired reStructuredText .rest answer and source aliases", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-rest-paired-"));

  try {
    const answerPath = join(tempDir, "answer.rest");
    const sourcePath = join(tempDir, "policy.rest");
    await Promise.all([
      writeFile(answerPath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(sourcePath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
...
test("verify accepts paired LaTeX answer and source exports", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-latex-paired-"));
  ...
  try {
    ...
    await Promise.all([
      writeFile(answerPath, "\\textbf{Parental leave}: Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(sourcePath, "\\section{Parental leave} Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
...
test("verify accepts paired XHT answer and source exports", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-xht-paired-"));

  try {
    const answerPath = join(tempDir, "answer.xht");
    const sourcePath = join(tempDir, "policy.xht");
    await Promise.all([
      writeFile(answerPath, "<p>Employees receive 12 weeks of paid parental leave.</p>\n", "utf8"),
      writeFile(
        sourcePath,
        "<html><head><title>Parental Leave Policy</title></head><body><p>Employees receive 12 weeks of paid parental leave.</p></body></html>\n",
        "utf8",
      ),
    ]);
...
test("verify-batch discovers XML answers from nested directories", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-xml-answers-"));

  try {
    const answerDir = join(tempDir, "answers");
    const sourceDir = join(tempDir, "sources");
    const answerPath = join(answerDir, "nested", "support-answer.xml");
...
test("evaluate writes a one-row-per-domain summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /^[^,\n]+,hr,33,0,0\.000,33,0,100,100,1\.000,100%,39,21,28,12,39,21,28,12$/m);
...
test("evaluate writes a one-row aggregate summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /,93,92,1,0,0\.000,277,277,1\.000,100%,hr .*105,60,79,33,105,60,79,33/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("verify discovers ini policy exports from a source directory", async () => {
  ...
  try {
    ...
    const sourcePath = join(sourceDir, "benefits.ini");

    await mkdir(sourceDir, { recursive: true });
    await Promise.all([
      writeFile(answerPath, "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(sourcePath, "[parental-leave]\nweeks=12\nEmployees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
...
test("verify normalizes direct JSONC answers and approved sources", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-jsonc-direct-"));

  try {
    const answerPath = join(tempDir, "answer.jsonc");
    const sourcePath = join(tempDir, "policy.jsonc");
...
test("verify normalizes direct uppercase JSON5 answers and approved sources", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-uppercase-json5-direct-"));
...
test("verify evaluates claims from direct INI answers and sources", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-ini-direct-"));

  try {
    const answerPath = join(tempDir, "answer.ini");
    const sourcePath = join(tempDir, "policy.ini");
    await Promise.all([
      writeFile(answerPath, "[policy]\nleave=Employees receive 12 weeks of paid parental leave.\n"),
      writeFile(sourcePath, "[policy]\nleave=Employees receive 12 weeks of paid parental leave.\n"),
    ]);
...
test("verify accepts direct AsciiDoc answers and sources with uppercase extensions", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-uppercase-asciidoc-"));
...
test("verify-batch discovers htm answers from answer directories", async () => {
  ...
  try {
    ...
    await Promise.all([
      writeFile(
        join(answerDir, "support-answer.htm"),
        "<html><body><p>Customers may request a refund within 30 days.</p></body></html>",
        "utf8",
      ),
    ...
    assert.equal(report.answerCount, 1);
    assert.equal(report.summary.verified, 1);
    assert.match(report.answers[0]?.answerPath ?? "", /support-answer\.htm$/);
    assert.deepEqual(
      report.answers[0]?.report.assessments.map((assessment) => assessment.claim.text),
...
test("verify excludes configured report outputs from source directory discovery", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-source-output-exclusion-"));

  try {
    const answerPath = join(tempDir, "answer.md");
    const sourceDir = join(tempDir, "sources");
    const reportPath = join(sourceDir, "report.json");
    await mkdir(sourceDir, { recursive: true });
    await writeFile(answerPath, "Employees receive 12 weeks of paid parental leave.\n", "utf8");
...
test("verify discovers JSON sources in source directories", async () => {
  ...
  try {
    ...
    await mkdir(sourceDir, { recursive: true });
    await writeFile(answerPath, "Employees receive 12 weeks of paid parental leave.\n", "utf8");
    await writeFile(join(sourceDir, "hr-policy.json"), '{"policy":"Employees receive 12 weeks of paid parental leave."}', "utf8");
...
test("verify-batch discovers uppercase JSONL answers recursively", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-uppercase-jsonl-answer-"));
  ...
  try {
    ...
    const report = JSON.parse(
      await runCli(["verify-batch", "--answer-dir", join(tempDir, "answers"), "--source", sourcePath, "--json"]),
    ) as {
      answerCount: number;
      summary: { verified: number };
      answers: Array<{ answerPath: string; report: { summary: { verified: number } } }>;
...
test("verify accepts a direct TOML approved source export", async () => {
  ...
  try {
    const answerPath = join(tempDir, "answer.md");
    const sourcePath = join(tempDir, "hr-policy.toml");
    await writeFile(answerPath, "Employees receive 12 weeks of paid parental leave.\n", "utf8");
    await writeFile(
      sourcePath,
      '[policy]\nleave = "Employees receive 12 weeks of paid parental leave."\n',
...
test("verify accepts direct .yml answer and approved source exports", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-direct-yml-"));

  try {
    const answerPath = join(tempDir, "answer.yml");
    const sourcePath = join(tempDir, "hr-policy.yml");
    await writeFile(answerPath, "policy:\n  leave: Employees receive 12 weeks of paid parental leave.\n", "utf8");
    await writeFile(sourcePath, "policy:\n  leave: Employees receive 12 weeks of paid parental leave.\n", "utf8");

    const report = JSON.parse(
      await runCli(["verify", "--answer", answerPath, "--source", sourcePath, "--json"]),
    ) as { answerPath: string; sources: Array<{ sourcePath: string; title: string }>; summary: { verified: number } };
...
test("verify accepts a direct NDJSON approved source export", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-ndjson-source-"));

  try {
    const answerPath = join(tempDir, "answer.md");
    const sourcePath = join(tempDir, "hr-policy.ndjson");
    await writeFile(answerPath, "Employees receive 12 weeks of paid parental leave.\n", "utf8");
    await writeFile(
      sourcePath,
      '{"policy":"Employees receive 12 weeks of paid parental leave."}\n',
...
test("verify accepts a direct LaTeX approved source export", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-latex-source-"));

  try {
    const answerPath = join(tempDir, "answer.md");
    const sourcePath = join(tempDir, "hr-policy.tex");
    await writeFile(answerPath, "Employees receive 12 weeks of paid parental leave.\n", "utf8");
    await writeFile(sourcePath, "\\textbf{Parental leave}: Employees receive 12 weeks of paid parental leave.\n", "utf8");
...
test("verify discovers YAML and YML sources in nested source directories", async () => {
  ...
  try {
    ...
    await writeFile(
      join(nestedSourceDir, "regional-policy.yml"),
...
test("verify discovers RFC 822 email sources in nested directories", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-email-source-directory-"));
  const answerPath = join(tempDir, "answer.md");
  const sourceDir = join(tempDir, "sources", "mailbox");
  const sourcePath = join(sourceDir, "refund-policy.eml");
...
test("verify discovers Apple Mail EMLX sources in nested directories", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-emlx-source-directory-"));
  const answerPath = join(tempDir, "answer.md");
  const sourceDir = join(tempDir, "sources", "mailbox");
  const sourcePath = join(sourceDir, "refund-policy.emlx");
...
test("review-queue scopes workload to a queue status", async () => {
  ...
  try {
    ...
    assert.deepEqual(pendingOverview.review, {
      ...
      pendingClaims: 128,
      reviewedClaims: 0,
      verdicts: { verified: 33, contradicted: 21, unsupported: 32, needs_review: 42 },
    });
