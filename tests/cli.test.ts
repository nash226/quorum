...
test("verify-batch discovers supported files in nested source and answer directories", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-nested-directories-"));

  try {
    const answerDir = join(tempDir, "answers");
    const sourceDir = join(tempDir, "sources");
    const answerPath = join(answerDir, "support", "answer.md");
    const sourcePath = join(sourceDir, "policies", "support.md");
