...
test("verify rejects source directories without supported files", async () => {
  ...
  try {
    ...
    await writeFile(join(sourceDir, "notes.csv"), "not a supported source\n", "utf8");

    await assert.rejects(
      runCli(["verify", "--answer", answerPath, "--source-dir", sourceDir]),
      new RegExp(`No source files found in ${escapeRegExp(sourceDir)}`),
    );
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
