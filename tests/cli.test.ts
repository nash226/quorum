...
test("verify-batch discovers answers in nested directories in stable order", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-nested-answer-dir-"));
  const answerDir = join(tempDir, "answers");
  const sourceDir = join(tempDir, "sources");

  try {
    await Promise.all([
      mkdir(join(answerDir, "hr"), { recursive: true }),
      mkdir(join(answerDir, "support", "regional"), { recursive: true }),
      mkdir(sourceDir),
      writeFile(join(sourceDir, "policy.md"), "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
    await Promise.all([
      writeFile(join(answerDir, "hr", "benefits.md"), "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
      writeFile(join(answerDir, "support", "regional", "benefits.md"), "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ]);
    ...
    assert.deepEqual(report.answers.map((answer) => answer.answerPath), [
      join(answerDir, "hr", "benefits.md"),
      join(answerDir, "support", "regional", "benefits.md"),
    ]);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
...
test("evaluate filters shipped example fixtures by domain", async () => {
  ...
  assert.match(stdout, /Fixtures: 31/);
});
...
test("evaluate writes a one-row-per-domain summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /^[^,\n]+,hr,31,0,0\.000,31,0,94,94,1\.000,100%,37,20,26,11,37,20,26,11$/m);
    assert.match(summaryCsv, /^[^,\n]+,support,59,0,0\.000,58,1,173,173,1\.000,100%,65,39,50,19,65,39,50,19$/m);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes the gate-aware result JSON to disk", async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.fixtureCount, 90);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("review-queue combines reviewer workload and benchmark drift", async () => {
  ...
  try {
    ...
    assert.equal(overview.evaluation.fixtureCount, 90);
    ...
    assert.match(text, /Benchmark drift: 0\/90 mismatches \(0%\)/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("review-queue scopes benchmark drift to selected domains", async () => {
  ...
  try {
    ...
    assert.equal(overview.evaluation.fixtureCount, 31);
    assert.deepEqual(overview.evaluation.domains, [{ domain: "hr", fixtureCount: 31, mismatchCount: 0 }]);
    const csvOutPath = join(tempDir, "queue.csv");
