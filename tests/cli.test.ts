...
test("evaluate writes a one-row-per-domain summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /^[^,\n]+,support,47,0,0\.000,46,1,137,137,1\.000,100%,51,32,39,15,51,32,39,15$/m);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes a one-row aggregate summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /,71,70,1,0,0\.000,210,210,1\.000,100%,hr \| support,24 \| 47,0 \| 0,0\.000 \| 0\.000,24 \| 46,0 \| 1,1\.000 \| 1\.000,100% \| 100%,80,48,58,24,80,48,58,24/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes the gate-aware result JSON to disk", async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.fixtureCount, 71);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("review-queue combines reviewer workload and benchmark drift", async () => {
  ...
  try {
    ...
    assert.equal(overview.evaluation.fixtureCount, 71);
    ...
    assert.match(text, /Benchmark drift: 0\/71 mismatches \(0%\)/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
