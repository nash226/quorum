...
test("verify discovers XHTML sources from a source directory", async () => {
  ...
  try {
    ...
    await mkdir(sourceDir, { recursive: true });
    await writeFile(answerPath, "Customers can request refunds within 30 days.\n", "utf8");
    await writeFile(
      join(sourceDir, "refund-policy.xhtml"),
    ...
    ) as { sources: Array<{ title: string }> };

    assert.deepEqual(report.sources.map((source) => source.title), ["Refund Policy"]);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
