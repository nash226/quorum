...
test("verify-batch discovers Quarto .qmd answers and sources", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-qmd-alias-"));
  ...
  try {
    ...
    await Promise.all([
      writeFile(join(answerDir, "leave.qmd"), "Employees receive 12 weeks of paid parental leave.\n", "utf8"),
    ...
    const report = JSON.parse(await runCli([
      ...
      "--json",
    ])) as { answerCount: number; sourceCount: number; answers: Array<{ answerPath: string }>; sources: Array<{ title: string }> };

    assert.equal(report.answerCount, 1);
    assert.equal(report.sourceCount, 1);
    assert.equal(report.answers[0]?.answerPath, join(answerDir, "leave.qmd"));
    assert.equal(report.sources[0]?.title, "hr-policy");
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
