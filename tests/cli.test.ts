...
test("help advertises the complete discovered input contract", async () => {
  ...
  assert.match(stdout, /Answers: Markdown\/MDX\/Quarto, AsciiDoc\/Org, MediaWiki, reStructuredText, LaTeX\/Textile, text\/log\/configuration, HTML\/XHTML, JSON\/JSONL, YAML, XML, CSV\/TSV, PDF, DOCX, and TOML/);
  assert.match(stdout, /Sources: Markdown\/MDX\/Quarto, AsciiDoc\/Org, MediaWiki, reStructuredText, LaTeX\/Textile, text\/log\/configuration, HTML\/XHTML, JSON\/JSONL, YAML, XML, CSV\/TSV, PDF, DOCX, and TOML/);
});
...
test("evaluate filters shipped example fixtures by domain", async () => {
  ...
  assert.match(stdout, /Fixtures: 29/);
});
...
test("evaluate writes a one-row-per-domain summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /^[^,\n]+,hr,29,0,0\.000,29,0,88,88,1\.000,100%,34,20,24,10,34,20,24,10$/m);
    assert.match(summaryCsv, /^[^,\n]+,support,54,0,0\.000,53,1,158,158,1\.000,100%,60,35,45,18,60,35,45,18$/m);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes a one-row aggregate summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /,83,82,1,0,0\.000,246,246,1\.000,100%,hr .*94,55,69,28,94,55,69,28/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes the gate-aware result JSON to disk", async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.fixtureCount, 83);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("review-queue scopes benchmark drift to selected domains", async () => {
  ...
  try {
    ...
    assert.equal(overview.evaluation.fixtureCount, 29);
    assert.deepEqual(overview.evaluation.domains, [{ domain: "hr", fixtureCount: 29, mismatchCount: 0 }]);
    const csvOutPath = join(tempDir, "queue.csv");
