...
test("verify-batch discovers nested .mdx answers", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-mdx-answer-dir-"));

  try {
    const answerDir = join(tempDir, "answers");
    const sourcePath = join(tempDir, "policy.md");
    const answerPath = join(answerDir, "nested", "answer.mdx");
