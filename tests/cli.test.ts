...
test("verify-batch discovers uppercase .TOML exports for answers and sources", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-toml-uppercase-"));
  ...
  try {
    ...
    await Promise.all([
      writeFile(join(answerDir, "leave.TOML"), 'leave = "Employees receive 12 weeks of paid parental leave."\n', "utf8"),
      writeFile(join(sourceDir, "hr-policy.TOML"), 'leave = "Employees receive 12 weeks of paid parental leave."\n', "utf8"),
    ]);
    ...
    assert.match(report.answers[0]?.answerPath ?? "", /answers\/nested\/leave\.TOML$/);
