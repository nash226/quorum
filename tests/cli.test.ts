...
test("evaluate filters shipped example fixtures by domain", async () => {
  ...
  assert.match(stdout, /Fixtures: 19/);
});
...
test("evaluate writes a one-row-per-domain summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /^[^,\n]+,hr,19,0,0\.000,19,0,58,58,1\.000,100%,24,12,15,7,24,12,15,7$/m);
    assert.match(summaryCsv, /^[^,\n]+,support,42,0,0\.000,41,1,124,124,1\.000,100%,46,30,35,13,46,30,35,13$/m);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes a one-row aggregate summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /,61,60,1,0,0\.000,182,182,1\.000,100%,hr \| support,19 \| 42,0 \| 0,0\.000 \| 0\.000,19 \| 41,0 \| 1,1\.000 \| 1\.000,100% \| 100%,70,42,50,20,70,42,50,20/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes the gate-aware result JSON to disk", async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.fixtureCount, 61);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("review-queue combines reviewer workload and benchmark drift", async () => {
  ...
  try {
    ...
    assert.equal(overview.review.totalAnswers, 26);
    assert.equal(overview.review.pendingAnswers, 25);
    assert.equal(overview.queueStatus, null);
    assert.equal(overview.evaluation.fixtureCount, 61);
