...
test("evaluate writes a one-row aggregate summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /,25,24,1,0,0\.000,71,71,1\.000,100%,hr \| support,12 \| 13,0 \| 0,0\.000 \| 0\.000,12 \| 12,0 \| 1,1\.000 \| 1\.000,100% \| 100%,28,21,18,4,28,21,18,4/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes the gate-aware result JSON to disk", async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.fixtureCount, 25);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
