...
test("evaluate writes a one-row-per-domain summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /^[^,\n]+,support,51,0,0\.000,50,1,149,149,1\.000,100%,56,34,43,16,56,34,43,16$/m);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes a one-row aggregate summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /,78,77,1,0,0\.000,231,231,1\.000,100%,hr .*88,53,65,25,88,53,65,25/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes the gate-aware result JSON to disk", async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.fixtureCount, 78);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("verify-batch discovers YAML and YML answers from answer directories", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "quorum-cli-yaml-answer-dir-"));
  ...
  try {
    ...
    await mkdir(answerDir, { recursive: true });
    await Promise.all([
      writeFile(join(answerDir, "yaml-answer.yaml"), "response: Customers can request refunds within 30 days.\n", "utf8"),
      writeFile(join(answerDir, "yml-answer.yml"), "response: Customers can request refunds within 30 days.\n", "utf8"),
    ...
    assert.deepEqual(
      ...
      [join(answerDir, "yaml-answer.yaml"), join(answerDir, "yml-answer.yml")],
    );
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
...
test("review-queue combines reviewer workload and benchmark drift", async () => {
  ...
  try {
    ...
    assert.equal(overview.evaluation.fixtureCount, 78);
    ...
    assert.match(text, /Benchmark drift: 0\/78 mismatches \(0%\)/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
