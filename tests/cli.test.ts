...
test("evaluate filters shipped example fixtures by domain", async () => {
  ...
  assert.match(stdout, /Fixtures: 11/);
});
...
test("evaluate writes a one-row-per-domain summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /^[^,\n]+,hr,11,0,0\.000,11,0,33,33,1\.000,100%,13,9,8,3,13,9,8,3$/m);
...
test("evaluate writes a one-row aggregate summary csv", async () => {
  ...
  try {
    ...
    assert.match(summaryCsv, /,24,23,1,0,0\.000,68,68,1\.000,100%,hr \| support,11 \| 13,0 \| 0,0\.000 \| 0\.000,11 \| 12,0 \| 1,1\.000 \| 1\.000,100% \| 100%,27,20,17,4,27,20,17,4/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
...
test("evaluate writes the gate-aware result JSON to disk", async () => {
  ...
  try {
    ...
    assert.equal(payload.summary.fixtureCount, 24);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
});
