...
test("preserves short punctuated claims with non-Latin letters", () => {
  const claims = extractClaims("休暇申請は承認制。\n지원 신청은 필수.");

  assert.deepEqual(claims.map((claim) => claim.text), ["休暇申請は承認制。", "지원 신청은 필수."]);
});

test("preserves short claims ending in Armenian and Ethiopic full stops", () => {
  const claims = extractClaims("Արձակուրդը վճարովի է։\nየእረፍት ፈቃድ ይፈቀዳል።");

  assert.deepEqual(claims.map((claim) => claim.text), [
    "Արձակուրդը վճարովի է։",
    "የእረፍት ፈቃድ ይፈቀዳል።",
  ]);
});
...
test("strips unicode checkbox markers from checklist answers", () => {
...
test("strips ballot-box X markers from Markdown task lists", () => {
  const claims = extractClaims(`- [☒] Employees receive 12 weeks of paid parental leave.
- [x] Healthcare coverage begins after 30 days of employment.`);
...
test("strips ballot-box-with-check markers from exported checklists", () => {
  const claims = extractClaims(`🗹 Employees receive 12 weeks of paid parental leave.
🗹 Healthcare coverage begins after 30 days of employment.`);
...
test("strips reference marks from exported checklist answers", () => {
  const claims = extractClaims(`※ Employees receive 12 weeks of paid parental leave.
⁕ Healthcare coverage begins after 30 days of employment.`);
...
test("strips directional bullets from localized answers", () => {
  const claims = extractClaims(`⁌ Employees receive 12 weeks of paid parental leave.
⁍ Healthcare coverage begins after 30 days of employment.`);
...
test("strips warning-sign bullets from exported checklists", () => {
  const claims = extractClaims(`⚠ Employees receive 12 weeks of paid parental leave.
⚠️ Healthcare coverage begins after 30 days of employment.`);
...
test("strips colored diamond bullets from rich-text exports", () => {
  const claims = extractClaims(`🔹 Employees receive 12 weeks of paid parental leave.
🔸 Healthcare coverage begins after 30 days of employment.`);
...
test("strips diamond bullets from exported policy answers", () => {
  const claims = extractClaims(`Policy notes:
...
test("strips CJK ideographic numbered-list markers from claims", () => {
  const claims = extractClaims(`Policy notes:

一、Employees receive 12 weeks of paid parental leave.
二、Healthcare coverage begins after 30 days of employment.
三、Contractors do not receive paid vacation.`);
...
test("strips parenthesized localized numbered-list markers", () => {
  const claims = extractClaims(`(١) Employees receive 12 weeks of paid parental leave.
(۲) Healthcare coverage begins after 30 days of employment.
(３) Contractors do not receive paid vacation.`);
...
test("strips unicode checkbox markers from exported task lists", () => {
  const claims = extractClaims(`☑ Employees receive 12 weeks of paid parental leave.
☐ Contractors do not receive paid vacation.
...
☒️ Managers approve exceptions within five business days.`);
...
test("extracts clean claims from fullwidth numeric list answers", () => {
  const claims = extractClaims(`Policy notes:
...
test("extracts claims from chevron bullets used by exported checklists", () => {
  const claims = extractClaims(`Policy notes:
...
test("strips small triangle bullets from exported policy answers", () => {
  const claims = extractClaims(`Policy notes:
...
test("extracts clean claims from fullwidth lettered markdown lists", () => {
  const claims = extractClaims(`Policy notes:

Ａ． Employees receive 12 weeks of paid parental leave.
ｂ） Healthcare coverage begins after 30 days of employment.
(Ｃ) Contractors do not receive paid vacation.`);
