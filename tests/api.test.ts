...
test("programmatic API returns mismatch metadata for fixture file evaluation helpers", async () => {
  ...
  assert.equal(batchResult.summary.mismatchCount, 0);
  assert.equal(batchResult.summary.matchedClaims, 202);
  assert.equal(batchResult.summary.totalExpectedClaims, 202);
  assert.equal(batchResult.summary.score, 1);
  assert.equal(batchResult.summary.scoreLabel, "100%");
  assert.deepEqual(batchResult.summary.domains, [
    {
      ...
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 28, contradicted: 15, unsupported: 19, needs_review: 8 },
      actualSummary: { verified: 28, contradicted: 15, unsupported: 19, needs_review: 8 },
    },
    {
      domain: "support",
      fixtureCount: 45,
