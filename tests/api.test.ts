...
test("programmatic API exposes supported source and answer extensions", () => {
  assert.deepEqual([...SOURCE_EXTENSIONS], [".md", ".markdown", ".mdx", ".qmd", ".adoc", ".asciidoc", ".org", ".mediawiki", ".wiki", ".rst", ".tex", ".textile", ".txt", ".text", ".log", ".ini", ".conf", ".cfg", ".properties", ".html", ".htm", ".xhtml", ".pdf", ".docx", ".json", ".jsonl", ".ndjson", ".yaml", ".yml", ".xml", ".toml", ".csv", ".tsv"]);
  assert.deepEqual([...ANSWER_EXTENSIONS], [".md", ".markdown", ".mdx", ".qmd", ".adoc", ".asciidoc", ".org", ".mediawiki", ".wiki", ".rst", ".tex", ".textile", ".txt", ".text", ".log", ".ini", ".conf", ".cfg", ".properties", ".html", ".htm", ".xhtml", ".pdf", ".docx", ".json", ".jsonl", ".ndjson", ".yaml", ".yml", ".xml", ".toml", ".csv", ".tsv"]);
});
...
test("programmatic API returns mismatch metadata for fixture file evaluation helpers", async () => {
  ...
  assert.equal(batchResult.shouldFail, false);
  assert.equal(batchResult.mismatchCount, 0);
  assert.equal(batchResult.summary.fixtureCount, 80);
  assert.equal(batchResult.summary.mismatchCount, 0);
  ...
  assert.equal(batchResult.scorecards.length, 80);
  assert.equal(singleResult.hasMismatch, false);
...
test("programmatic API exports batch evaluation helpers", async () => {
  ...
  assert.match(rendered, /Fixtures: 80/);
