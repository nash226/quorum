...
test("programmatic API returns mismatch metadata for fixture file evaluation helpers", async () => {
  ...
  assert.equal(batchResult.shouldFail, false);
  assert.equal(batchResult.mismatchCount, 0);
  assert.equal(batchResult.summary.fixtureCount, 26);
  assert.equal(batchResult.summary.mismatchCount, 0);
  assert.equal(batchResult.summary.matchedClaims, 74);
  assert.equal(batchResult.summary.totalExpectedClaims, 74);
  assert.equal(batchResult.summary.score, 1);
  assert.equal(batchResult.summary.scoreLabel, "100%");
  assert.deepEqual(batchResult.summary.domains, [
    {
      ...
      answersWithoutClaims: 0,
      matchedClaims: 36,
      totalExpectedClaims: 36,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 14, contradicted: 10, unsupported: 9, needs_review: 3 },
      actualSummary: { verified: 14, contradicted: 10, unsupported: 9, needs_review: 3 },
    },
    {
      domain: "support",
      fixtureCount: 14,
      ...
      matchedClaims: 38,
      totalExpectedClaims: 38,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 15, contradicted: 12, unsupported: 10, needs_review: 1 },
      actualSummary: { verified: 15, contradicted: 12, unsupported: 10, needs_review: 1 },
    },
  ]);
  assert.equal(batchResult.scorecards.length, 26);
  assert.equal(singleResult.hasMismatch, false);
...
test("programmatic API exports batch evaluation helpers", async () => {
  ...
  assert.match(rendered, /Fixtures: 26/);
