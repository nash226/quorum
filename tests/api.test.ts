...
test("programmatic API exposes supported source and answer extensions", () => {
  assert.deepEqual([...SOURCE_EXTENSIONS], [".md", ".markdown", ".mdown", ".mkdn", ".mdx", ".qmd", ".adoc", ".asciidoc", ".org", ".mediawiki", ".wiki", ".rst", ".tex", ".textile", ".txt", ".text", ".log", ".ini", ".properties", ".html", ".htm", ".xhtml", ".pdf", ".docx", ".json", ".jsonl", ".ndjson", ".yaml", ".yml", ".xml", ".toml", ".csv", ".tsv"]);
  assert.deepEqual([...ANSWER_EXTENSIONS], [".md", ".markdown", ".mdown", ".mkdn", ".mdx", ".qmd", ".adoc", ".asciidoc", ".org", ".mediawiki", ".wiki", ".rst", ".tex", ".textile", ".txt", ".text", ".log", ".ini", ".properties", ".html", ".htm", ".xhtml", ".pdf", ".docx", ".json", ".jsonl", ".ndjson", ".yaml", ".yml", ".xml", ".toml", ".csv", ".tsv"]);
});
...
test("programmatic API returns mismatch metadata for fixture file evaluation helpers", async () => {
  ...
  assert.equal(batchResult.shouldFail, false);
  assert.equal(batchResult.mismatchCount, 0);
  assert.equal(batchResult.summary.fixtureCount, 85);
  assert.equal(batchResult.summary.mismatchCount, 0);
  ...
  assert.deepEqual(batchResult.summary.domains, [
    ...
    },
    {
      ...
      mismatchCount: 0,
      mismatchRate: 0,
      answersWithClaims: 54,
      answersWithoutClaims: 1,
      matchedClaims: 161,
      totalExpectedClaims: 161,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 61, contradicted: 36, unsupported: 46, needs_review: 18 },
      actualSummary: { verified: 61, contradicted: 36, unsupported: 46, needs_review: 18 },
    },
  ]);
  assert.equal(batchResult.scorecards.length, 85);
  assert.equal(singleResult.hasMismatch, false);
...
test("programmatic API exports batch evaluation helpers", async () => {
  ...
  assert.match(rendered, /Fixtures: 85/);
