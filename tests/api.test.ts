...
test("programmatic API returns mismatch metadata for fixture file evaluation helpers", async () => {
  ...
  assert.equal(batchResult.shouldFail, false);
  assert.equal(batchResult.mismatchCount, 0);
  assert.equal(batchResult.summary.fixtureCount, 55);
  assert.equal(batchResult.summary.mismatchCount, 0);
  assert.equal(batchResult.summary.matchedClaims, 164);
  assert.equal(batchResult.summary.totalExpectedClaims, 164);
  assert.equal(batchResult.summary.score, 1);
  assert.equal(batchResult.summary.scoreLabel, "100%");
  assert.deepEqual(batchResult.summary.domains, [
    ...
    },
    {
      ...
      mismatchCount: 0,
      mismatchRate: 0,
      answersWithClaims: 36,
      answersWithoutClaims: 1,
      matchedClaims: 109,
      totalExpectedClaims: 109,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 40, contradicted: 28, unsupported: 30, needs_review: 11 },
      actualSummary: { verified: 40, contradicted: 28, unsupported: 30, needs_review: 11 },
    },
  ]);
  assert.equal(batchResult.scorecards.length, 55);
  assert.equal(singleResult.hasMismatch, false);
...
test("programmatic API exports batch evaluation helpers", async () => {
  ...
  assert.match(rendered, /Fixtures: 55/);
