...
test("programmatic API returns mismatch metadata for fixture file evaluation helpers", async () => {
  ...
  assert.equal(batchResult.shouldFail, false);
  assert.equal(batchResult.mismatchCount, 0);
  assert.equal(batchResult.summary.fixtureCount, 30);
  assert.equal(batchResult.summary.mismatchCount, 0);
  assert.equal(batchResult.summary.matchedClaims, 86);
  assert.equal(batchResult.summary.totalExpectedClaims, 86);
  assert.equal(batchResult.summary.score, 1);
  assert.equal(batchResult.summary.scoreLabel, "100%");
  assert.deepEqual(batchResult.summary.domains, [
    {
      ...
      mismatchCount: 0,
      mismatchRate: 0,
      answersWithClaims: 15,
      answersWithoutClaims: 0,
      matchedClaims: 45,
      totalExpectedClaims: 45,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 18, contradicted: 12, unsupported: 12, needs_review: 3 },
      actualSummary: { verified: 18, contradicted: 12, unsupported: 12, needs_review: 3 },
    },
    {
      domain: "support",
      fixtureCount: 15,
      mismatchCount: 0,
      mismatchRate: 0,
      answersWithClaims: 14,
      answersWithoutClaims: 1,
      matchedClaims: 41,
      totalExpectedClaims: 41,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 16, contradicted: 13, unsupported: 11, needs_review: 1 },
      actualSummary: { verified: 16, contradicted: 13, unsupported: 11, needs_review: 1 },
    },
  ]);
  assert.equal(batchResult.scorecards.length, 30);
  assert.equal(singleResult.hasMismatch, false);
...
test("programmatic API exports batch evaluation helpers", async () => {
  ...
  assert.match(rendered, /Fixtures: 30/);
