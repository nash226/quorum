...
test("programmatic API exposes supported source and answer extensions", () => {
  assert.deepEqual([...SOURCE_EXTENSIONS], [".md", ".markdown", ".mdx", ".adoc", ".asciidoc", ".org", ".txt", ".html", ".htm", ".xhtml", ".pdf", ".docx", ".json", ".yaml", ".yml", ".xml", ".toml", ".mediawiki", ".wiki"]);
  assert.deepEqual([...ANSWER_EXTENSIONS], [".md", ".markdown", ".mdx", ".adoc", ".asciidoc", ".org", ".mediawiki", ".wiki", ".txt", ".html", ".htm", ".xhtml", ".pdf", ".docx", ".toml"]);
});
...
test("programmatic API returns mismatch metadata for fixture file evaluation helpers", async () => {
  ...
  assert.equal(batchResult.shouldFail, false);
  assert.equal(batchResult.mismatchCount, 0);
  assert.equal(batchResult.summary.fixtureCount, 78);
  assert.equal(batchResult.summary.mismatchCount, 0);
  assert.equal(batchResult.summary.matchedClaims, 231);
  assert.equal(batchResult.summary.totalExpectedClaims, 231);
  assert.equal(batchResult.summary.score, 1);
  assert.equal(batchResult.summary.scoreLabel, "100%");
  assert.deepEqual(batchResult.summary.domains, [
    ...
    },
    {
      ...
      mismatchCount: 0,
      mismatchRate: 0,
      answersWithClaims: 50,
      answersWithoutClaims: 1,
      matchedClaims: 149,
      totalExpectedClaims: 149,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 56, contradicted: 34, unsupported: 43, needs_review: 16 },
      actualSummary: { verified: 56, contradicted: 34, unsupported: 43, needs_review: 16 },
    },
  ]);
  assert.equal(batchResult.scorecards.length, 78);
  assert.equal(singleResult.hasMismatch, false);
...
test("programmatic API exports batch evaluation helpers", async () => {
  ...
  assert.equal(scorecards.length, 78);
  assert.equal(scorecards.some(hasEvaluationMismatch), false);
  assert.match(rendered, /Fixtures: 78/);
