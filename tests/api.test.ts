...
test("programmatic API exposes supported source and answer extensions", () => {
  assert.deepEqual([...SOURCE_EXTENSIONS], [".md", ".markdown", ".mdx", ".qmd", ".adoc", ".asciidoc", ".org", ".mediawiki", ".wiki", ".rst", ".tex", ".textile", ".txt", ".text", ".log", ".ini", ".cfg", ".conf", ".properties", ".html", ".htm", ".xhtml", ".pdf", ".docx", ".json", ".jsonl", ".ndjson", ".yaml", ".yml", ".xml", ".toml", ".csv", ".tsv"]);
  assert.deepEqual([...ANSWER_EXTENSIONS], [".md", ".markdown", ".mdx", ".qmd", ".adoc", ".asciidoc", ".org", ".mediawiki", ".wiki", ".rst", ".tex", ".textile", ".txt", ".text", ".log", ".ini", ".cfg", ".conf", ".properties", ".html", ".htm", ".xhtml", ".pdf", ".docx", ".json", ".jsonl", ".ndjson", ".yaml", ".yml", ".xml", ".toml", ".csv", ".tsv"]);
});
...
test("programmatic API returns mismatch metadata for fixture file evaluation helpers", async () => {
  ...
  assert.equal(batchResult.shouldFail, false);
  assert.equal(batchResult.mismatchCount, 0);
  assert.equal(batchResult.summary.fixtureCount, 82);
  assert.equal(batchResult.summary.mismatchCount, 0);
  assert.equal(batchResult.summary.matchedClaims, 243);
  assert.equal(batchResult.summary.totalExpectedClaims, 243);
  assert.equal(batchResult.summary.score, 1);
  assert.equal(batchResult.summary.scoreLabel, "100%");
  assert.deepEqual(batchResult.summary.domains, [
    {
      ...
      mismatchCount: 0,
      mismatchRate: 0,
      answersWithClaims: 28,
      answersWithoutClaims: 0,
      matchedClaims: 85,
      totalExpectedClaims: 85,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 33, contradicted: 20, unsupported: 23, needs_review: 9 },
      actualSummary: { verified: 33, contradicted: 20, unsupported: 23, needs_review: 9 },
    },
    {
      domain: "support",
      fixtureCount: 54,
  ...
  assert.equal(batchResult.scorecards.length, 82);
  assert.equal(singleResult.hasMismatch, false);
...
test("programmatic API exports batch evaluation helpers", async () => {
  ...
  assert.match(rendered, /Fixtures: 82/);
