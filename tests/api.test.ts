...
test("programmatic API exposes supported source and answer extensions", () => {
  assert.deepEqual([...SOURCE_EXTENSIONS], [".md", ".markdown", ".mdown", ".mkdn", ".mdwn", ".mdx", ".qmd", ".adoc", ".asciidoc", ".org", ".org-mode", ".mediawiki", ".wiki", ".rst", ".rest", ".tex", ".textile", ".txt", ".text", ".log", ".ini", ".properties", ".conf", ".cfg", ".html", ".htm", ".xhtml", ".pdf", ".docx", ".json", ".jsonl", ".ndjson", ".json5", ".jsonc", ".yaml", ".yml", ".xml", ".toml", ".csv", ".tsv"]);
  assert.deepEqual([...ANSWER_EXTENSIONS], [".md", ".markdown", ".mdown", ".mkdn", ".mdwn", ".mdx", ".qmd", ".adoc", ".asciidoc", ".org", ".org-mode", ".mediawiki", ".wiki", ".rst", ".rest", ".tex", ".textile", ".txt", ".text", ".log", ".ini", ".properties", ".conf", ".cfg", ".html", ".htm", ".xhtml", ".pdf", ".docx", ".json", ".jsonl", ".ndjson", ".json5", ".jsonc", ".yaml", ".yml", ".xml", ".toml", ".csv", ".tsv"]);
});
...
test("programmatic API returns mismatch metadata for fixture file evaluation helpers", async () => {
  ...
  assert.equal(batchResult.shouldFail, false);
  assert.equal(batchResult.mismatchCount, 0);
  assert.equal(batchResult.summary.fixtureCount, 89);
  assert.equal(batchResult.summary.mismatchCount, 0);
  assert.equal(batchResult.summary.matchedClaims, 264);
  assert.equal(batchResult.summary.totalExpectedClaims, 264);
  assert.equal(batchResult.summary.score, 1);
  assert.equal(batchResult.summary.scoreLabel, "100%");
  assert.deepEqual(batchResult.summary.domains, [
    ...
    },
    {
      ...
      mismatchCount: 0,
      mismatchRate: 0,
      answersWithClaims: 58,
      answersWithoutClaims: 1,
      matchedClaims: 173,
      totalExpectedClaims: 173,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 65, contradicted: 39, unsupported: 50, needs_review: 19 },
      actualSummary: { verified: 65, contradicted: 39, unsupported: 50, needs_review: 19 },
    },
  ]);
  assert.equal(batchResult.scorecards.length, 89);
  assert.equal(singleResult.hasMismatch, false);
...
test("programmatic API exports batch evaluation helpers", async () => {
  ...
  assert.match(rendered, /Fixtures: 89/);
