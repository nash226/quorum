...
test("programmatic API exposes supported source and answer extensions", () => {
  assert.deepEqual([...SOURCE_EXTENSIONS], [".md", ".markdown", ".mdx", ".qmd", ".adoc", ".asciidoc", ".org", ".mediawiki", ".wiki", ".rst", ".tex", ".textile", ".txt", ".text", ".log", ".ini", ".properties", ".html", ".htm", ".xhtml", ".pdf", ".docx", ".json", ".jsonl", ".ndjson", ".yaml", ".yml", ".xml", ".toml", ".csv", ".tsv"]);
  assert.deepEqual([...ANSWER_EXTENSIONS], [".md", ".markdown", ".mdx", ".qmd", ".adoc", ".asciidoc", ".org", ".mediawiki", ".wiki", ".rst", ".tex", ".textile", ".txt", ".text", ".log", ".ini", ".properties", ".html", ".htm", ".xhtml", ".pdf", ".docx", ".json", ".jsonl", ".ndjson", ".yaml", ".yml", ".xml", ".toml", ".csv", ".tsv"]);
});
...
test("HTTP API exposes CORS preflight metadata for every POST endpoint", async () => {
  const api = await startApiServer({ host: "127.0.0.1", port: 0 });
...
test("programmatic batch verification normalizes JSONL answers", async () => {
  const result = await verifyAnswerBatchContentsResult({
    answers: [{
      answerPath: "answers/remote-work.jsonl",
...
test("programmatic batch verification normalizes TSV answers and sources", async () => {
  const result = await verifyAnswerBatchContentsResult({
    answers: [{
      answerPath: "answers/leave-answer.tsv",
      answer: "field\nEmployees receive 12 weeks of paid leave.\n",
    }],
    sources: [{
      sourcePath: "policies/leave-policy.tsv",
      content: "policy\nEmployees receive 12 weeks of paid leave.\n",
    }],
  });
  ...
  assert.equal(result.report.answers[0]?.answerPath, "answers/leave-answer.tsv");
  assert.equal(result.report.sources[0]?.sourcePath, "policies/leave-policy.tsv");
});
...
test("programmatic API returns mismatch metadata for fixture file evaluation helpers", async () => {
  ...
  assert.equal(batchResult.shouldFail, false);
  assert.equal(batchResult.mismatchCount, 0);
  assert.equal(batchResult.summary.fixtureCount, 84);
  assert.equal(batchResult.summary.mismatchCount, 0);
  assert.equal(batchResult.summary.matchedClaims, 249);
  assert.equal(batchResult.summary.totalExpectedClaims, 249);
  assert.equal(batchResult.summary.score, 1);
  assert.equal(batchResult.summary.scoreLabel, "100%");
  assert.deepEqual(batchResult.summary.domains, [
    {
      ...
      mismatchCount: 0,
      mismatchRate: 0,
      answersWithClaims: 30,
      answersWithoutClaims: 0,
      matchedClaims: 91,
      totalExpectedClaims: 91,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 36, contradicted: 20, unsupported: 25, needs_review: 10 },
      actualSummary: { verified: 36, contradicted: 20, unsupported: 25, needs_review: 10 },
    },
    {
      domain: "support",
      fixtureCount: 54,
      mismatchCount: 0,
      mismatchRate: 0,
      answersWithClaims: 53,
      answersWithoutClaims: 1,
      matchedClaims: 158,
      totalExpectedClaims: 158,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 60, contradicted: 35, unsupported: 45, needs_review: 18 },
      actualSummary: { verified: 60, contradicted: 35, unsupported: 45, needs_review: 18 },
    },
  ]);
  assert.equal(batchResult.scorecards.length, 84);
  assert.equal(singleResult.hasMismatch, false);
...
test("programmatic API accepts vendor JSON content types for POST endpoints", async () => {
  const api = await startApiServer({ host: "127.0.0.1", port: 0 });
  ...
  try {
    const response = await fetch(`${api.url}/extract-claims`, {
      method: "POST",
      headers: {
        "content-type": "application/vnd.quorum.preview+json; charset=utf-8",
        "X-Quorum-Request-Id": "vendor-json-content-type-check",
      },
      body: JSON.stringify({ answer: "Employees receive 12 weeks of paid parental leave." }),
    });

    assert.equal(response.status, 200);
    assert.equal(response.headers.get("x-quorum-request-id"), "vendor-json-content-type-check");
    const payload = (await response.json()) as { requestId: string; answerHasClaims: boolean };
    assert.equal(payload.requestId, "vendor-json-content-type-check");
    assert.equal(payload.answerHasClaims, true);
  } finally {
    await api.close();
  }
...
test("programmatic API reports no-claims reviewer queue slices", async () => {
  const api = await startApiServer({ host: "127.0.0.1", port: 0 });
  ...
  try {
    const response = await fetch(`${api.url}${REVIEW_QUEUE_PATH}`, {
      ...
      body: JSON.stringify({
        ...
        reviewCsvContent: [
          "answer_label,answer_path,answer_preview,answer_has_claims,claim_id,claim_text,model_verdict,model_reason,evidence_titles,evidence_quotes,reviewer_verdict,reviewer_notes",
          "Empty answer,answers/empty.md,Empty.,false,,,,No claims were extracted from this answer.,,,,",
          "Pending answer,answers/pending.md,Pending.,true,claim_1,Pending policy claim.,needs_review,Needs review,HR Policy,Pending policy claim.,,",
        ].join("\n"),
      }),
    });
...
test("programmatic API exports batch evaluation helpers", async () => {
  ...
  assert.match(rendered, /Fixtures: 84/);
