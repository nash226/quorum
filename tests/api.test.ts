...
test("programmatic API returns mismatch metadata for fixture file evaluation helpers", async () => {
  ...
  assert.equal(batchResult.shouldFail, false);
  assert.equal(batchResult.mismatchCount, 0);
  assert.equal(batchResult.summary.fixtureCount, 54);
  assert.equal(batchResult.summary.mismatchCount, 0);
  assert.equal(batchResult.summary.matchedClaims, 161);
  assert.equal(batchResult.summary.totalExpectedClaims, 161);
  assert.equal(batchResult.summary.score, 1);
  assert.equal(batchResult.summary.scoreLabel, "100%");
  assert.deepEqual(batchResult.summary.domains, [
    ...
    },
    {
      ...
      mismatchCount: 0,
      mismatchRate: 0,
      answersWithClaims: 35,
      answersWithoutClaims: 1,
      matchedClaims: 106,
      totalExpectedClaims: 106,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 39, contradicted: 27, unsupported: 29, needs_review: 11 },
      actualSummary: { verified: 39, contradicted: 27, unsupported: 29, needs_review: 11 },
    },
  ]);
  assert.equal(batchResult.scorecards.length, 54);
  assert.equal(singleResult.hasMismatch, false);
...
test("programmatic API exports batch evaluation helpers", async () => {
  ...
  assert.match(rendered, /Fixtures: 54/);
