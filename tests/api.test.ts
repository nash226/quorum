...
test("programmatic API returns mismatch metadata for fixture file evaluation helpers", async () => {
  ...
  assert.equal(batchResult.shouldFail, false);
  assert.equal(batchResult.mismatchCount, 0);
  assert.equal(batchResult.summary.fixtureCount, 20);
  assert.equal(batchResult.summary.mismatchCount, 0);
  assert.equal(batchResult.summary.matchedClaims, 56);
  assert.equal(batchResult.summary.totalExpectedClaims, 56);
  assert.equal(batchResult.summary.score, 1);
  assert.equal(batchResult.summary.scoreLabel, "100%");
  assert.deepEqual(batchResult.summary.domains, [
    {
      ...
      answersWithoutClaims: 0,
      matchedClaims: 21,
      totalExpectedClaims: 21,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 9, contradicted: 5, unsupported: 5, needs_review: 2 },
      actualSummary: { verified: 9, contradicted: 5, unsupported: 5, needs_review: 2 },
    },
    {
      domain: "support",
      fixtureCount: 13,
      mismatchCount: 0,
      mismatchRate: 0,
      answersWithClaims: 12,
      answersWithoutClaims: 1,
      matchedClaims: 35,
      totalExpectedClaims: 35,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 14, contradicted: 11, unsupported: 9, needs_review: 1 },
      actualSummary: { verified: 14, contradicted: 11, unsupported: 9, needs_review: 1 },
    },
  ]);
  assert.equal(batchResult.scorecards.length, 20);
  assert.equal(singleResult.hasMismatch, false);
...
test("programmatic API exports batch evaluation helpers", async () => {
  ...
  assert.equal(scorecards.length, 20);
  assert.equal(scorecards.some(hasEvaluationMismatch), false);
  assert.match(rendered, /Fixtures: 20/);
