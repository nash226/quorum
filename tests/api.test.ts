...
test("programmatic API returns mismatch metadata for fixture file evaluation helpers", async () => {
  ...
  assert.equal(batchResult.shouldFail, false);
  assert.equal(batchResult.mismatchCount, 0);
  assert.equal(batchResult.summary.fixtureCount, 69);
  assert.equal(batchResult.summary.mismatchCount, 0);
  assert.equal(batchResult.summary.matchedClaims, 205);
  assert.equal(batchResult.summary.totalExpectedClaims, 205);
  assert.equal(batchResult.summary.score, 1);
  assert.equal(batchResult.summary.scoreLabel, "100%");
  assert.deepEqual(batchResult.summary.domains, [
    {
      ...
      mismatchCount: 0,
      mismatchRate: 0,
      answersWithClaims: 24,
      answersWithoutClaims: 0,
      matchedClaims: 73,
      totalExpectedClaims: 73,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 29, contradicted: 16, unsupported: 19, needs_review: 9 },
      actualSummary: { verified: 29, contradicted: 16, unsupported: 19, needs_review: 9 },
    },
    {
      domain: "support",
      fixtureCount: 45,
  ...
  assert.equal(batchResult.scorecards.length, 69);
  assert.equal(singleResult.hasMismatch, false);
...
test("programmatic API exports batch evaluation helpers", async () => {
  ...
  assert.match(rendered, /Fixtures: 69/);
