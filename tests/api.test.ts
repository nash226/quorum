...
test("programmatic API returns mismatch metadata for fixture file evaluation helpers", async () => {
  ...
  assert.deepEqual(batchResult.summary.domains, [
    ...
    },
    {
      ...
      mismatchCount: 0,
      mismatchRate: 0,
      answersWithClaims: 55,
      answersWithoutClaims: 1,
      matchedClaims: 164,
      totalExpectedClaims: 164,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 62, contradicted: 37, unsupported: 47, needs_review: 18 },
      actualSummary: { verified: 62, contradicted: 37, unsupported: 47, needs_review: 18 },
    },
  ]);
  assert.equal(batchResult.scorecards.length, 86);
  assert.equal(singleResult.hasMismatch, false);
...
test("programmatic API exports batch evaluation helpers", async () => {
  ...
  assert.match(rendered, /Fixtures: 86/);
