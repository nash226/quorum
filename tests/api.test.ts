...
test("programmatic API exposes supported source and answer extensions", () => {
  assert.deepEqual([...SOURCE_EXTENSIONS], [".md", ".markdown", ".mdown", ".mkdn", ".mdwn", ".mdx", ".qmd", ".adoc", ".asciidoc", ".org", ".org-mode", ".mediawiki", ".wiki", ".rst", ".rest", ".tex", ".textile", ".txt", ".text", ".log", ".ini", ".properties", ".conf", ".cfg", ".html", ".htm", ".xhtml", ".pdf", ".docx", ".rtf", ".json", ".jsonl", ".ndjson", ".json5", ".jsonc", ".yaml", ".yml", ".xml", ".toml", ".csv", ".tsv", ".eml", ".emlx"]);
  assert.deepEqual([...ANSWER_EXTENSIONS], [".md", ".markdown", ".mdown", ".mkdn", ".mdwn", ".mdx", ".qmd", ".adoc", ".asciidoc", ".org", ".org-mode", ".mediawiki", ".wiki", ".rst", ".rest", ".tex", ".textile", ".txt", ".text", ".log", ".ini", ".properties", ".conf", ".cfg", ".html", ".htm", ".xhtml", ".pdf", ".docx", ".rtf", ".json", ".jsonl", ".ndjson", ".json5", ".jsonc", ".yaml", ".yml", ".xml", ".toml", ".csv", ".tsv", ".eml", ".emlx"]);
});
...
test("programmatic batch verification normalizes uppercase CSV answers and sources", async () => {
  const result = await verifyAnswerBatchContentsResult({
    answers: [{
      answerPath: "answers/leave-answer.CSV",
      answer: "field\nEmployees receive 12 weeks of paid leave.\n",
    }],
    sources: [{
      sourcePath: "policies/leave-policy.CSV",
      content: "policy\nEmployees receive 12 weeks of paid leave.\n",
    }],
  });
  ...
  assert.equal(result.report.answers[0]?.answerPath, "answers/leave-answer.CSV");
  assert.equal(result.report.sources[0]?.sourcePath, "policies/leave-policy.CSV");
});
...
test("programmatic API returns mismatch metadata for fixture file evaluation helpers", async () => {
  ...
  assert.equal(batchResult.shouldFail, false);
  assert.equal(batchResult.mismatchCount, 0);
  assert.equal(batchResult.summary.fixtureCount, 93);
  assert.equal(batchResult.summary.mismatchCount, 0);
  assert.equal(batchResult.summary.matchedClaims, 277);
  assert.equal(batchResult.summary.totalExpectedClaims, 277);
  assert.equal(batchResult.summary.score, 1);
  assert.equal(batchResult.summary.scoreLabel, "100%");
  assert.deepEqual(batchResult.summary.domains, [
    {
      ...
      mismatchCount: 0,
      mismatchRate: 0,
      answersWithClaims: 33,
      answersWithoutClaims: 0,
      matchedClaims: 100,
      totalExpectedClaims: 100,
      score: 1,
      scoreLabel: "100%",
      expectedSummary: { verified: 39, contradicted: 21, unsupported: 28, needs_review: 12 },
      actualSummary: { verified: 39, contradicted: 21, unsupported: 28, needs_review: 12 },
    },
    {
      domain: "support",
      fixtureCount: 60,
  ...
  assert.equal(batchResult.scorecards.length, 93);
  assert.equal(singleResult.hasMismatch, false);
...
test("programmatic API exports batch evaluation helpers", async () => {
  ...
  assert.match(rendered, /Fixtures: 93/);
