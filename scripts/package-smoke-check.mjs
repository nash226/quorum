...
const cliConfigAliasPackageDir = mkdtempSync(join(tmpdir(), "quorum-package-cli-config-aliases-"));
try {
  for (const extension of ["conf", "cfg"]) {
    const answerPath = join(cliConfigAliasPackageDir, `answer.${extension}`);
    const sourcePath = join(cliConfigAliasPackageDir, `policy.${extension}`);
    writeFileSync(answerPath, "claim=Employees receive 12 weeks of paid parental leave.\n");
    writeFileSync(sourcePath, "policy=Employees receive 12 weeks of paid parental leave.\n");
    ...
    if (
      ...
      payload.answerPath !== answerPath ||
      payload.sources?.[0]?.title !== "policy" ||
      payload.sources?.[0]?.sourcePath !== sourcePath
    ) {
      throw new Error(`Package artifact did not verify the expected .${extension} answer/source contract.`);
    }
  }
} finally {
  rmSync(cliConfigAliasPackageDir, { recursive: true, force: true });
}
...
try {
  ...
  if (
    ...
    throw new Error("Package artifact did not verify the expected TOML answer contract.");
  }
} finally {
...
const cliMediaWikiAliasPackageDir = mkdtempSync(join(tmpdir(), "quorum-package-cli-mediawiki-alias-"));
try {
  const answerPath = join(cliMediaWikiAliasPackageDir, "answer.mediawiki");
  const sourcePath = join(cliMediaWikiAliasPackageDir, "policy.wiki");
  writeFileSync(answerPath, "Employees receive 12 weeks of paid parental leave.\n");
  writeFileSync(sourcePath, "Employees receive 12 weeks of paid parental leave.\n");
  ...
  if (
    mediaWikiPayload.summary?.verified !== 1 ||
    mediaWikiPayload.answerPath !== answerPath ||
    mediaWikiPayload.sources?.[0]?.title !== "policy" ||
    mediaWikiPayload.sources?.[0]?.sourcePath !== sourcePath
  ) {
    throw new Error("Package artifact did not verify the expected MediaWiki answer/source contract.");
  }
} finally {
  rmSync(cliMediaWikiAliasPackageDir, { recursive: true, force: true });
}
