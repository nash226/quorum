...
try {
  for (const extension of ["org", "org-mode"]) {
    const answerPath = join(cliOrgPackageDir, `answer.${extension}`);
    const sourcePath = join(cliOrgPackageDir, `policy.${extension}`);
    writeFileSync(answerPath, "Employees receive 12 weeks of paid parental leave.\n");
    writeFileSync(sourcePath, "* Parental Leave Policy\n\nEmployees receive 12 weeks of paid parental leave.\n");
    ...
    if (
      ...
      throw new Error(`Package artifact did not verify the expected .${extension} Org-mode answer/source contract.`);
    }
  }
} finally {
