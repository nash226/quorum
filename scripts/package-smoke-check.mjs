...
try {
  for (const extension of ["rst", "rest"]) {
    const answerPath = join(cliRstPackageDir, `answer.${extension}`);
    const sourcePath = join(cliRstPackageDir, `policy.${extension}`);
    writeFileSync(answerPath, "Employees receive 12 weeks of paid parental leave.\n");
    writeFileSync(sourcePath, "Parental Leave Policy\n=====================\n\nEmployees receive 12 weeks of paid parental leave.\n");
    ...
    if (
      ...
      throw new Error(`Package artifact did not verify the expected .${extension} reStructuredText contract.`);
    }
  }
} finally {
