...
function sourceTitleFromPath(sourcePath: string): string {
  return basename(sourcePath).replace(/\.(?:md|markdown|mdown|mdx|qmd|adoc|asciidoc|org|mediawiki|wiki|rst|tex|textile|txt|text|log|ini|properties|html?|xhtml|pdf|docx|jsonl?|ndjson|xml|ya?ml|toml|csv|tsv)$/i, "");
}
