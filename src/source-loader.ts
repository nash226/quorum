...
function isJsonSource(sourcePath: string): boolean {
  return /\.json(?:l|ld)?$/i.test(sourcePath);
}
...
function sourceTitleFromPath(sourcePath: string): string {
  return basename(sourcePath).replace(/\.(?:md|markdown|mdx|adoc|asciidoc|org|mediawiki|wiki|rst|tex|txt|text|html?|xhtml|pdf|docx|json(?:l|ld)?|xml|ya?ml|toml)$/i, "");
}
