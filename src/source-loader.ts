...
function sourceTitleFromPath(sourcePath: string): string {
  return basename(sourcePath).replace(/\.(?:md|markdown|mdown|mkdn|mdx|qmd|adoc|asciidoc|org(?:-mode)?|mediawiki|wiki|rst|tex|textile|txt|text|log|ini|properties|html?|xhtml|pdf|docx|jsonl?|ndjson|xml|ya?ml|toml|csv|tsv)$/i, "");
}
