...
function sourceTitleFromPath(sourcePath: string): string {
  return basename(sourcePath).replace(/\.(?:md|markdown|mdx|qmd|adoc|asciidoc|org|mediawiki|wiki|rst|tex|textile|txt|text|log|ini|conf|cfg|properties|html?|xhtml|pdf|docx|jsonl?|ndjson|xml|ya?ml|toml|csv|tsv)$/i, "");
}
