...
function sourceTitleFromPath(sourcePath: string): string {
  return basename(sourcePath).replace(/\.(?:md|markdown|mdx|qmd|mdown|mkdn|adoc|asciidoc|org|mediawiki|wiki|rst|tex|txt|text|ini|properties|html?|xhtml|pdf|docx|jsonl?|xml|ya?ml|toml|csv|tsv)$/i, "");
}
