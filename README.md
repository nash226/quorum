...
When explicit `--answer` or `--source` paths are combined with a directory,
Quorum keeps the explicit paths first, appends discovered files in sorted path
order, and removes duplicate paths. This makes repeated CI runs produce stable
source and answer ordering while still allowing a curated file to be promoted
ahead of a directory-wide scan.
