...
and text import outputs group claims by `answer_path` so batch review handoffs
stay organized by answer file.
...
- `verify-batch --summary-csv-out <path>`: write one CSV row per answer with an `answer_preview`, verdict totals, fail-policy status, the verdicts that triggered it, and the approved source metadata used for that batch run
...
- `import-review --review-csv <path>`: import a filled reviewer decision CSV and
  ...
  also preserve `answer_path` context
