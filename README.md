...
JSON/JSONL/YAML/XML/TOML, CSV/TSV, RFC 822 email (`.eml`), PDF, and DOCX
...
reviewed without a separate conversion step. RFC 822 email sources use the
message subject as the source title, the message date as freshness metadata
when present, and the email body as evidence. Use `--source-dir` for a mixed
