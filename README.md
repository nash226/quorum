...
Claim extraction now recognizes Myanmar full-stop sentence terminators, keeping
Myanmar policy statements separate for evidence matching.
...
The same packaged smoke gate now verifies the `.rest` reStructuredText alias
...
The packaged CLI smoke gate also verifies direct `.org-mode` answers and
approved sources, keeping this Org mode alias covered for installed workflows.
...
The packaged smoke gate now verifies `npm run evaluate -- ... --result-json`,
keeping installed evaluation integrations aligned with the gate-aware score,
mismatch, and threshold result contract.
...
Claim extraction now strips colored diamond bullets such as `🔹` and `🔸` from
rich-text policy exports, keeping those statements atomic before evidence
verification.
...
Claim extraction also recognizes hollow-circle bullets such as `○` from
...
Evaluation score gates now document and preserve the distinction between
...
Claim extraction also splits substantial Latin- and Arabic-semicolon-delimited policy claims and normalizes bracketed, Arabic-Indic, Persian, fullwidth, and CJK ideographic ordered-list
...
Common `.conf` and `.cfg` configuration exports now follow the same discovery
and evidence path as `.ini` and `.properties` files.
...
- Direct `verify` also accepts Org-mode answer exports, preserving the answer
  path in the JSON report for reviewer handoff.
- Direct `verify` also accepts Org-mode approved-source exports, preserving the
  source path and fallback title in the JSON evidence report.
- Direct `verify` and recursive answer discovery also accept CSV answer
  ...
  workflow.
- Direct `verify` also accepts MediaWiki approved-source exports, preserving
  the source path in evidence for reviewer handoff.
- Direct `verify` also accepts CSV approved-source exports, preserving the
...
The current default branch passes this gate with 802 automated tests, keeping
the documented local workflow aligned with the repository's CI checks. The
latest verification also covers the versioned format-discovery contract and
the packaged CLI/API smoke paths described above.
...
`npm run check` is the recommended pre-PR gate and matches the repository CI
workflow: it runs tests, the TypeScript build, HTTP and packaged-artifact smoke
checks, and the checked-in evaluation suite in one command.
...
so editor metadata and hidden staging folders are not treated as inputs, and
supported extensions are matched case-insensitively for exports such as
`POLICY.MD`. The
...
`.org-mode`, `.mediawiki`, `.wiki`, `.tex`, `.textile`), plain text and
configuration exports (`.txt`, `.text`, `.log`, `.ini`, `.properties`, `.conf`,
`.cfg`, `.env`), exported HTML/MHTML/XHTML (`.html`, `.htm`, `.xht`, `.xhtml`,
`.mht`, `.mhtml`), PDF (`.pdf`), Word documents (`.docx`), RTF (`.rtf`),
JSON/JSONL/NDJSON (`.json`, `.jsonl`, `.ndjson`, `.json5`, `.jsonc`), TOML
(`.toml`), YAML/YML (`.yaml`, `.yml`), XML (`.xml`), and CSV/TSV (`.csv`,
`.tsv`), plus RFC 822 email exports (`.eml`, `.emlx`).
...
The packaged CLI smoke gate also verifies direct JSONL approved-source
verification, preserving the source path and fallback title at the published
artifact boundary.
...
Because stdin can only be consumed once, a batch command cannot combine
`--answer -` with `--source -`; stream one side and provide the other as files.
...
exactly one data row for each of the 85 checked-in benchmark fixtures.
...
The benchmark inventory is currently reconciled at 85 fixtures, including the
