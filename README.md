...
AsciiDoc/Org/MediaWiki/LaTeX/Textile (`.adoc`, `.asciidoc`, `.org`,
`.org-mode`, `.mediawiki`, `.wiki`, `.tex`, `.textile`), plain text and
configuration exports (`.txt`, `.text`, `.log`, `.ini`, `.properties`, `.conf`,
`.cfg`), exported HTML/XHTML (`.html`, `.htm`, `.xhtml`), PDF (`.pdf`), Word
documents (`.docx`), JSON-family exports (`.json`, `.jsonl`, `.ndjson`, `.json5`,
`.jsonc`), TOML (`.toml`), YAML/YML (`.yaml`, `.yml`), XML (`.xml`), and
CSV/TSV (`.csv`, `.tsv`).
